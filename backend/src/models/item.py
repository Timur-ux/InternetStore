...
class Item(Base):
    ...
    item_id = Column(Integer, primary_key=True, index=True, autoincrement=True)   # Unique item identifier
    item_name = Column(String(100), nullable=False)             # Item name
    uri = Column(String(255), nullable=False)                   # URL for item data
    item_category_id = Column(Integer, nullable=False)       # Unique category identifier
